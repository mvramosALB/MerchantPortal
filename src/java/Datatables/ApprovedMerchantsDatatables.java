 ...
 */
public class ApprovedMerchantsDatatables extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        String[] columnNames = {"MERCHNAME","MerchType","RATETYPE","RATEVALUE","qr_exp","ApprovedBy","ApprovedDate"};
...
//        }
        ...
        try {
            ...
            Logger.getLogger(ApprovedMerchantsDatatables.class.getName()).log(Level.SEVERE, null, ex);
        }
    ...
    public int getTotalRecordCount() throws SQLException, ClassNotFoundException {
        ...
        int totalRecords = -1;
        String sql = "SELECT " + "COUNT(*) as count " + "FROM " + " vw_approved_merchants";
    ...
    public JSONObject getPersonDetails(int totalRecords, HttpServletRequest request)
        ...
        con = DBConnection.getConnection();
        String sql = "select * from vw_approved_merchants";

        String globeSearch = " where (Merchname collate SQL_Latin1_General_CP1_CI_AS like '%" + GLOBAL_SEARCH_TERM + "%' "
                + "or merchtype collate SQL_Latin1_General_CP1_CI_AS like '%" + GLOBAL_SEARCH_TERM + "%'"
                + "or RateType collate SQL_Latin1_General_CP1_CI_AS like '%" + GLOBAL_SEARCH_TERM + "%')";
        ...
        while (rs.next()) {
            ...
            ja.put(rs.getString("ApprovedBy"));//8
            ja.put(rs.getString("ApprovedDate"));//9
            ja.put(rs.getString("ProcessStatus"));//10
            ja.put(rs.getString("ActiveStatus"));//11
            array.put(ja);
        }
        stmt.close();
        rs.close();
        String query = "SELECT " + "COUNT(*) as count " + "FROM " + " vw_approved_merchants" + " " + whereClause;
        //for pagination
        if (GLOBAL_SEARCH_TERM != "") {
            query += searchSQL;
