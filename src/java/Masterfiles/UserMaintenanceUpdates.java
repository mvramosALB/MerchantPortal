 ...
 */
public class UserMaintenanceUpdates extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (UserGroup.equals("7")) {
            String[] TerminalIDArray = request.getParameterValues("Terminal");
            TerminalID = Arrays.toString(TerminalIDArray).replace("[", "").replace("]", "").replace(" ", "");
        } else {
            TerminalID = request.getParameter("Terminal");
        }
        System.out.println("Terminals  : "+TerminalID);
        ...
        if (action == 1) {
            ...
            if (!U.CheckUserExist(UserName)) {
                        ...
                        String Password = Hashing.getShai256(PasswordGenerated);
                        ...
                        if (U.AddUserInfo(UserIDCreated, MobileNumber, Email, CreatedBy, MerchID, BranchID, TerminalID)) {
                            ...
                            if (PassNotif.equals("SMS")) {
                                SMS.SendSMS(MobileNumber, "Your Temporary Password is :" + PasswordGenerated, CreatedBy,"RisingTide");
                            } else {
                                try {
                                    Sending.SendEmail.SendEmail(1, Email, FullName, "Your Temporary Password is <b>" + PasswordGenerated + "</b>", "", "Disclaimer", "Footer", "Merchant Portal", UserID);
                                } catch (ClassNotFoundException ex) {
                                    Logger.getLogger(UserMaintenanceUpdates.class.getName()).log(Level.SEVERE, null, ex);
                                }
                            }
